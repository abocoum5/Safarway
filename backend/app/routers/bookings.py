...
COMMISSION_RATE = 10  # 10 MRU fixe par place reservee
...
def creer_reservation(
    ...
    if current_user.role not in [models.UserRole.voyageur, models.UserRole.admin]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Seuls les voyageurs peuvent reserver")
    ...
    if trip.available_seats < booking_data.seats_booked:
        raise HTTPException(status_code=400, detail=f"Places insuffisantes ({trip.available_seats})")
    ...
    while db.query(models.Booking).filter(models.Booking.reference_code == reference).first():
        reference = generate_reference()
    ...
    # Infos chauffeur uniquement apres reservation confirmee
    new_booking.driver_phone = trip.driver.phone if trip.driver else None
    new_booking.driver_name = trip.driver.name if trip.driver else None

    # SMS de confirmation avec numero chauffeur
    try:
        ...
        send_booking_confirmation(current_user.phone, {
            ...
            "driver_phone": trip.driver.phone if trip.driver else "",
...
def annuler_reservation(
    ...
    if booking.status == models.BookingStatus.annule:
        raise HTTPException(status_code=400, detail="Deja annulee")
